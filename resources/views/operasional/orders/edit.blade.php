...
@php
    $storedWorkEvidences = collect($order->evidence_work_paths ?? [])->filter()->map(fn ($path) => [
        'path' => $path,
    ...
    $storedPaymentEvidences = collect($order->evidence_payment_paths ?? [])->filter()->map(fn ($path) => [
        'path' => $path,
        'url' => \Illuminate\Support\Facades\Storage::disk('r2')->url($path),
        'is_image' => in_array(\Illuminate\Support\Str::lower(pathinfo($path, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'webp'], true),
        'name' => basename($path),
    ...
    <div class="card card-flush mb-7">
        <div class="card-header pt-5">
            ...
            <div class="card-toolbar">
                <span class="badge badge-light-primary fs-7">{{ $order->order_number }}</span>
            </div>
        </div>
        <div class="card-body pt-0">
            <div class="row mb-5">
                <div class="col-md-4 fv-row">
                    ...
                    <input type="text" name="order_date" id="order_date" class="form-control" placeholder="Pilih tanggal" value="{{ $order->order_date->format('Y-m-d') }}" required />
                    </div>
                </div>
                    ...
                    <select id="bank_account_id" class="form-select bank-account-select" data-placeholder="Pilih Bank/Cash">
                        ...
                        @foreach($bankAccounts as $account)
                            <option value="{{ $account->id }}"
                                ...
                                @selected((string) $order->bank_account_id === (string) $account->id)>
        ...
        <div class="card-body pt-0">
            <div class="row">
                <div class="col-md-6">
                    ...
                    @if($storedWorkEvidences->isNotEmpty())
                        <div class="mt-3">
                            <div class="fw-semibold text-gray-600 fs-7 mb-2">Eviden Tersimpan</div>
                            <div class="row g-2">
                                @foreach($storedWorkEvidences as $file)
                                    <div class="col-auto">
                                        @if($file['is_image'])
                                            <button type="button" class="btn p-0 border-0 evidence-preview-open" data-url="{{ $file['url'] }}" title="Perbesar gambar">
                                                <img src="{{ $file['url'] }}" class="rounded border" style="width:72px;height:72px;object-fit:cover;" alt="Eviden pekerjaan" />
                                            </button>
                                        @else
                ...
                <div class="col-md-6">
                    ...
                    @if($storedPaymentEvidences->isNotEmpty())
                        <div class="mt-3">
                            <div class="fw-semibold text-gray-600 fs-7 mb-2">Eviden Tersimpan</div>
                            <div class="row g-2">
                                @foreach($storedPaymentEvidences as $file)
                                    <div class="col-auto">
                                        @if($file['is_image'])
                                            <button type="button" class="btn p-0 border-0 evidence-preview-open" data-url="{{ $file['url'] }}" title="Perbesar gambar">
                                                <img src="{{ $file['url'] }}" class="rounded border" style="width:72px;height:72px;object-fit:cover;" alt="Eviden pembayaran" />
                                            </button>
                                        @else
                                            <a href="{{ $file['url'] }}" target="_blank" class="btn btn-sm btn-light-primary">{{ $file['name'] }}</a>
                                        @endif
        ...
        <div class="card-body">
            <div class="d-flex flex-wrap justify-content-end align-items-end gap-3">
                ...
                <button type="button" class="btn btn-primary" id="btnSaveOrder">
                    <i class="ki-duotone ki-check-circle fs-3"><span class="path1"></span><span class="path2"></span></i> Update Order
                </button>
            </div>
        </div>
    </div>
...
@php
    $initialOrderPayload = [
        ...
        'vehicle_size' => $order->vehicle?->vehicle_size,
        'mileage' => $order->mileage,
        'km_service' => $order->km_service,
        'km_return' => $order->km_return,
        'head_mechanic' => $order->head_mechanic,
...
var isEditMode = true;
var orderSubmitUrl = @json(route('orders.update', $order));
var orderSubmitMethod = 'PUT';
var initialOrder = @json($initialOrderPayload);
...
$(document).ready(function() {
    ...
    $('#order_date').flatpickr({
        ...
        altInput: true,
        altFormat: 'd/m/Y',
        defaultDate: @json($order->order_date->format('Y-m-d'))
    });
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                ...
                data.forEach(function(order) {
                    ...
                    body.append('<tr><td class="fw-bold">' + order.order_number + '</td><td>' + order.order_date + '</td><td>Rp ' + formatNumber(parseFloat(order.total)) + '</td><td>' + statusBadge + '</td></tr>');
                });
            }
        });
    });
