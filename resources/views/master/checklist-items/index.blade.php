...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><span class="bullet bg-gray-500 w-5px h-2px"></span></li>
    <li class="breadcrumb-item text-muted">Item Checklist</li>
@endsection
...
@section('toolbar_actions')
    ...
    <button type="button" class="btn btn-sm btn-primary" onclick="openCreateModal()">
        <i class="ki-duotone ki-plus-square fs-3"><span class="path1"></span><span class="path2"></span><span class="path3"></span></i> Tambah Item
    </button>
    @endcan
@endsection
    ...
    <div class="card-header border-0 pt-6">
        ...
        <div class="card-toolbar">
            <div class="d-flex align-items-center position-relative">
                ...
                <input type="text" id="searchInput" class="form-control form-control w-250px ps-12" placeholder="Cari item..." />
            </div>
        </div>
    </div>
        ...
        <table id="kt_table" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>Rp. {{ number_format($item->price_small, 0, ',', '.') }}</td>
                    <td>Rp. {{ number_format($item->price_medium, 0, ',', '.') }}</td>
                    <td>Rp. {{ number_format($item->price_large, 0, ',', '.') }}</td>
                    <td>
                        @if($item->is_active)
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold" id="modalTitle">Tambah Item</h2>
            ...
            <form id="dataForm">
                <div class="modal-body scroll-y mx-5 my-7" style="max-height:60vh">
                    ...
                    <div class="fv-row mb-5">
                        ...
                        <select name="checklist_category_id" id="f_category" class="form-select form-select" required>
                    ...
                    <div class="fv-row mb-5">
                        <label class="required form-label fw-semibold">Nama Item</label>
                        <input type="text" name="name" id="f_name" class="form-control form-control" required />
                    </div>
                    <div class="row mb-5">
                        <div class="col-md-4 fv-row">
                            <label class="required form-label fw-semibold">Harga Small</label>
                            <div class="input-group">
                                <span class="input-group-text">Rp.</span>
                                <input type="text" id="f_price_small_display" class="form-control price-display" data-target="f_price_small" placeholder="0" aria-label="Harga Small" inputmode="numeric" autocomplete="off" required />
                            </div>
                            <input type="hidden" name="price_small" id="f_price_small" />
                        </div>
                        <div class="col-md-4 fv-row">
                            <label class="required form-label fw-semibold">Harga Medium</label>
                            <div class="input-group">
                                <span class="input-group-text">Rp.</span>
                                <input type="text" id="f_price_medium_display" class="form-control price-display" data-target="f_price_medium" placeholder="0" aria-label="Harga Medium" inputmode="numeric" autocomplete="off" required />
                            </div>
                            <input type="hidden" name="price_medium" id="f_price_medium" />
                        </div>
                        <div class="col-md-4 fv-row">
                            <label class="required form-label fw-semibold">Harga Large</label>
                            <div class="input-group">
                                <span class="input-group-text">Rp.</span>
                                <input type="text" id="f_price_large_display" class="form-control price-display" data-target="f_price_large" placeholder="0" aria-label="Harga Large" inputmode="numeric" autocomplete="off" required />
                            </div>
...
$(document).ready(function() {
    table = $('#kt_table').DataTable({
        ...
        buttons: [
            {
                extend: 'excelHtml5',
                title: 'Item Checklist - Garasi Hobby',
...
function setPriceValue(field, value) {
    var digits = normalizePriceInput(value);
    document.getElementById(field).value = digits;
    document.getElementById(field + '_display').value = formatRupiah(digits);
}
...
function openEditModal(id) {
    ...
    fetch('/master/checklist-items/' + id + '/edit', {
        headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(r => r.json())
    .then(data => {
        document.getElementById('modalTitle').textContent = 'Edit Item';
        document.getElementById('f_category').value = data.checklist_category_id || '';
...
function deleteItem(id, name) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch('/master/checklist-items/' + id, {
                method: 'DELETE',
                headers: {
...
document.getElementById('dataForm').addEventListener('submit', function(e) {
    ...
    var url = formMode === 'create' ? '{{ route("checklist-items.store") }}' : '/master/checklist-items/' + editId;
