...
<div class="card card-flush">
    <div class="card-header border-0 pt-6">
        <div class="card-title"><span class="text-gray-600">Kelola kategori transaksi yang bisa dipakai untuk uang masuk maupun uang keluar.</span></div>
    ...
    <div class="card-body pt-0">
        <table id="kt_table" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
            <thead><tr class="fw-semibold fs-6 text-gray-800"><th>No</th><th>Kode</th><th>Nama</th><th>Deskripsi</th><th>Jumlah Item</th><th class="text-end">Aksi</th></tr></thead>
            <tbody>
...
function openEditModal(id){ fetch('/master/finance-categories/'+id+'/edit').then(r=>r.json()).then(d=>{ editId=id; $('#modalTitle').text('Edit Kategori'); $('#f_code').val(d.code); $('#f_name').val(d.name); $('#f_description').val(d.description); errors({}); new bootstrap.Modal(document.getElementById('formModal')).show(); }); }
