...
<div class="card card-flush gh-table-card">
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <span class="text-gray-600">Kelola kategori untuk pembelian aset bengkel.</span>
        </div>
        <div class="card-toolbar position-relative">
            <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-5"><span class="path1"></span><span class="path2"></span></i>
            <input id="searchInput" class="form-control w-250px ps-12" placeholder="Cari kategori aset..." />
        </div>
    </div>
            ...
            <table id="kt_table" class="table gh-table align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->asset_purchases_count }} transaksi</td>
                        <td class="text-end">
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="modalTitle">Tambah Kategori Aset</h2>
...
<script>
var editId = null;
var table = $('#kt_table').DataTable({order: [], columnDefs: [{orderable:false, targets:[0,5]}]});
...
function openCreateModal(){ editId=null; $('#modalTitle').text('Tambah Kategori Aset'); $('#dataForm')[0].reset(); errors({}); new bootstrap.Modal(document.getElementById('formModal')).show(); }
function openEditModal(id){ fetch('/master/asset-categories/'+id+'/edit').then(r=>r.json()).then(d=>{ editId=id; $('#modalTitle').text('Edit Kategori Aset'); $('#f_code').val(d.code); $('#f_name').val(d.name); $('#f_description').val(d.description); errors({}); new bootstrap.Modal(document.getElementById('formModal')).show(); }); }
$('#dataForm').on('submit', function(e){ e.preventDefault(); var fd=new FormData(this); if(editId) fd.append('_method','PUT'); fetch(editId?'/master/asset-categories/'+editId:'{{ route('asset-categories.store') }}',{method:'POST',headers:{'X-CSRF-TOKEN':$('meta[name=csrf-token]').attr('content'),'Accept':'application/json'},body:fd}).then(r=>r.json()).then(d=>d.success?location.reload():errors(d)); });
function deleteItem(id,name){ Swal.fire({title:'Hapus kategori aset?',text:name,icon:'warning',showCancelButton:true,confirmButtonText:'Ya, hapus'}).then(x=>{if(x.isConfirmed) fetch('/master/asset-categories/'+id,{method:'DELETE',headers:{'X-CSRF-TOKEN':$('meta[name=csrf-token]').attr('content'),'Accept':'application/json'}}).then(r=>r.json()).then(d=>d.success?location.reload():Swal.fire('Gagal',d.message,'error'));});}
</script>
@endpush
