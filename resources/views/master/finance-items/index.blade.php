...
<thead><tr class="fw-semibold fs-6 text-gray-800"><th>No</th><th>Kode</th><th>Nama Item</th><th>Kategori</th><th>Status</th><th class="text-end">Aksi</th></tr></thead><tbody>
...
<div class="mb-5"><label class="required form-label">Kategori</label><select name="finance_category_id" id="f_category" class="form-select" required><option value="">-- Pilih Kategori --</option>@foreach($categories as $category)<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach</select></div>
...
@push('scripts')
<script>
var editId=null; var table=$('#kt_table').DataTable({order:[],columnDefs:[{orderable:false,targets:[0,5]}]}); $('#searchInput').on('keyup',function(){table.search(this.value).draw();});
