...
@section('title', 'Edit Revenue Sharing')
...
<a href="{{ route('revenue-sharings.show', $revenueSharing) }}" class="btn btn-sm btn-light"><i class="ki-duotone ki-left fs-3"></i> Kembali</a>
@endsection

@section('content')
@php
    $months = collect(range(1, 12))->mapWithKeys(fn($m) => [$m => \Carbon\Carbon::create(null, $m, 1)->translatedFormat('F')]);
    $selectedType = old('cutoff_type', $revenueSharing->cutoff_type);
    $selectedYear = old('cutoff_year', $revenueSharing->cutoff_year);
    $selectedMonth = old('cutoff_month', $revenueSharing->cutoff_month);
    $selectedQuarter = old('cutoff_quarter', $revenueSharing->cutoff_quarter);
@endphp
<div class="card card-flush revenue-form-card">
    <div class="card-header border-0 pt-6"><div class="card-title"><h2 class="fw-bold">Edit Sharing Revenue</h2></div></div>
    <form method="POST" action="{{ route('revenue-sharings.update', $revenueSharing) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <input type="hidden" name="revenue_cutoff_id" value="{{ old('revenue_cutoff_id', $revenueSharing->revenue_cutoff_id) }}">
        <div class="card-body pt-0">
            @if($errors->any())<div class="alert alert-danger"><ul class="mb-0">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif
            <div class="revenue-cutoff-box mb-7">
                <div><div class="text-muted fs-8 text-uppercase fw-semibold mb-1">Cut Off Revenue</div><div class="fw-bolder fs-3 text-gray-900">{{ $cutoff['label'] }}</div><div class="text-muted">Revenue bersih periode ini: <span class="fw-bold text-success">Rp {{ number_format($cutoff['netRevenue'], 0, ',', '.') }}</span></div></div>
                <div class="text-end"><div class="fw-bold fs-4 text-primary">Rp {{ number_format($cutoff['grossRevenue'], 0, ',', '.') }}</div><div class="text-muted fs-8">Revenue kotor</div></div>
            </div>
            <div class="row g-5">
                ...
                <div class="col-md-6"><label class="required form-label fw-semibold">Nama Penerima</label><input name="recipient_name" class="form-control" value="{{ old('recipient_name', $revenueSharing->recipient_name) }}" required></div>
                <div class="col-md-3"><label class="required form-label fw-semibold">Persentase Pembagian</label><div class="input-group"><input type="number" step="0.01" min="0" max="100" name="sharing_percentage" id="sharingPercentage" class="form-control" value="{{ old('sharing_percentage', $revenueSharing->sharing_percentage) }}" required><span class="input-group-text">%</span></div></div>
                ...
                <div class="col-12"><label class="required form-label fw-semibold">Akun Bank</label><select name="bank_account_id" class="form-select bank-account-select" required><option value="">-- Pilih Bank --</option>@foreach($bankAccounts as $bank)<option value="{{ $bank->id }}" data-logo-url="{{ $bank->logo_url }}" data-logo-text="{{ $bank->logo_text }}" data-bank-name="{{ $bank->bank_name }}" data-bank-code="{{ $bank->code }}" data-bank-balance="Rp {{ number_format($bank->balance,0,',','.') }}" @selected(old('bank_account_id', $revenueSharing->bank_account_id)==$bank->id)>{{ $bank->bank_name }}</option>@endforeach</select></div>
                <div class="col-12"><label class="form-label fw-semibold">Eviden R2 Cloudflare Baru</label><input type="file" name="evidence[]" id="evidenceInput" class="form-control" accept=".jpg,.jpeg,.png,.webp,.pdf" multiple><div class="form-text">Kosongkan jika eviden lama tetap dipakai.</div></div>
                <div class="col-12"><div id="evidencePreview" class="d-none border rounded p-3"><div class="fw-semibold mb-3">Preview Eviden Baru</div><div class="row g-3 preview-list"></div></div></div>
                <div class="col-12"><label class="form-label fw-semibold">Catatan</label><textarea name="notes" class="form-control" rows="3">{{ old('notes', $revenueSharing->notes) }}</textarea></div>
            </div>
        </div>
        <div class="card-footer d-flex justify-content-end"><a href="{{ route('revenue-sharings.show', $revenueSharing) }}" class="btn btn-light me-3">Batal</a><button class="btn btn-primary"><i class="ki-duotone ki-send fs-3"><span class="path1"></span><span class="path2"></span></i> Update & Ajukan</button></div>
    </form>
</div>
...
@push('scripts')
<script>
var netRevenue={{ (float) $cutoff['netRevenue'] }};function formatNumber(v){return Math.round(v||0).toString().replace(/\B(?=(\d{3})+(?!\d))/g,'.');}function syncSharing(){var pct=parseFloat(document.getElementById('sharingPercentage').value||0);document.getElementById('sharingAmountDisplay').value=formatNumber(Math.max(0,netRevenue)*(pct/100));}function syncCutoffFields(){var type=document.getElementById('cutoffType').value;document.querySelector('.cutoff-month').classList.toggle('d-none',type!=='monthly');document.querySelector('.cutoff-quarter').classList.toggle('d-none',type!=='quarterly');}document.getElementById('sharingPercentage').addEventListener('input',syncSharing);document.getElementById('cutoffType').addEventListener('change',syncCutoffFields);syncSharing();syncCutoffFields();
function bankOptionTemplate(option){if(!option.id)return option.text;var el=option.element;var logoUrl=el.getAttribute('data-logo-url');var logoText=el.getAttribute('data-logo-text')||'BNK';var bankName=el.getAttribute('data-bank-name')||option.text;var bankCode=el.getAttribute('data-bank-code')||'';var balance=el.getAttribute('data-bank-balance')||'';var logo=logoUrl?'<img src="'+logoUrl+'" alt="'+bankName.replace(/"/g,'&quot;')+'">':'<span>'+logoText+'</span>';return $('<div class="bank-select-option"><div class="bank-select-logo">'+logo+'</div><div class="bank-select-text"><div class="name">'+bankCode+' - '+bankName+'</div><div class="meta">Saldo '+balance+'</div></div></div>');}$('.bank-account-select').select2({width:'100%',templateResult:bankOptionTemplate,templateSelection:bankOptionTemplate,escapeMarkup:function(markup){return markup;}});
