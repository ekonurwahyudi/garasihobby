...
@section('toolbar_actions')
    <a href="{{ route('finance-transactions.show', $finance_transaction) }}" class="btn btn-sm btn-light-primary">
...
<div class="card card-flush">
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <h2 class="fw-bold">Edit Input Keuangan</h2>
        </div>
    </div>
    <form id="dataForm" method="POST" action="{{ route('finance-transactions.update', $finance_transaction) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card-body pt-0">
            ...
            <div class="row g-5 mb-5">
                <div class="col-md-6">
                    ...
                    <select name="transaction_type" id="transactionType" class="form-select" required>
                        ...
                        <option value="income" @selected(old('transaction_type', $finance_transaction->transaction_type) === 'income')>Uang Masuk</option>
                        <option value="expense" @selected(old('transaction_type', $finance_transaction->transaction_type) === 'expense')>Uang Keluar</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="required form-label fw-semibold">Tanggal Kwitansi</label>
                    <input type="text" name="transaction_date" id="transactionDate" class="form-control" placeholder="Pilih tanggal" value="{{ old('transaction_date', $finance_transaction->transaction_date?->format('Y-m-d')) }}" required />
                </div>
            </div>
            ...
            <div class="row g-5 mb-5">
                <div class="col-md-8">
                    ...
                    <input type="text" name="activity" class="form-control" value="{{ old('activity', $finance_transaction->activity ?: $finance_transaction->description) }}" required />
                </div>
                <div class="col-md-4">
                    <label class="required form-label fw-semibold">Nominal</label>
                    <div class="input-group">
                        <span class="input-group-text">Rp</span>
                        <input type="text" id="amountDisplay" class="form-control" inputmode="numeric" autocomplete="off" value="{{ number_format((float) old('amount', $finance_transaction->amount), 0, ',', '.') }}" required />
                    </div>
                    <input type="hidden" name="amount" id="amount" value="{{ old('amount', $finance_transaction->amount) }}" />
                </div>
            </div>
            ...
            <div class="row g-5 mb-5">
                <div class="col-md-6">
                    ...
                    <select name="finance_category_id" id="categorySelect" class="form-select" required>
                        ...
                        <option value="{{ $category->id }}" @selected((string) old('finance_category_id', $finance_transaction->item?->finance_category_id) === (string) $category->id)>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    ...
                    <select name="finance_item_id" id="itemSelect" class="form-select" required>
                        ...
                        <option value="{{ $item->id }}" data-category="{{ $item->finance_category_id }}" @selected((string) old('finance_item_id', $finance_transaction->finance_item_id) === (string) $item->id)>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row g-5 mb-5">
                <div class="col-md-6">
                    ...
                    <select name="bank_account_id" class="form-select bank-account-select" data-placeholder="Pilih Bank/Cash" required>
                        ...
                        <option value="{{ $account->id }}"
                            ...
                            @selected((string) old('bank_account_id', $finance_transaction->bank_account_id) === (string) $account->id)>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-text">Kosongkan jika tidak ingin mengganti eviden lama.</div>
                </div>
            </div>

            @if($evidenceFiles->isNotEmpty())
            <div class="mb-5">
                <label class="form-label fw-semibold">Eviden Saat Ini</label>
                <div class="row g-3">
                    @foreach($evidenceFiles as $evidence)
                    <div class="col-md-6 col-xl-4">
                        <a href="{{ $evidence->url }}" target="_blank" class="finance-evidence-thumb d-block">
                            <div class="finance-evidence-frame mb-3">
                                @if($evidence->is_image)
                                    <img src="{{ $evidence->url }}" alt="Eviden transaksi {{ $loop->iteration }}">
                                @else
                                    <span class="badge badge-light-danger">PDF</span>
                                @endif
                            </div>
                            <div class="fw-semibold text-gray-800 text-truncate">{{ $evidence->name ?? 'Eviden ' . $loop->iteration }}</div>
                        </a>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="fv-row mb-5">
                ...
                <textarea name="notes" class="form-control" rows="3">{{ old('notes', $finance_transaction->notes) }}</textarea>
            </div>
        </div>
        <div class="card-footer d-flex justify-content-end">
            <a href="{{ route('finance-transactions.show', $finance_transaction) }}" class="btn btn-light me-3">
                <i class="ki-duotone ki-cross fs-3"><span class="path1"></span><span class="path2"></span></i> Batal
            </a>
            <button type="submit" class="btn btn-primary">
                <i class="ki-duotone ki-send fs-3"><span class="path1"></span><span class="path2"></span></i> Update & Ajukan
            </button>
        </div>
    </form>
</div>
...
.finance-evidence-thumb{border:1px solid #e4e8f0;border-radius:12px;background:#fff;padding:12px;height:100%;transition:transform .15s ease,box-shadow .15s ease}
.finance-evidence-thumb:hover{transform:translateY(-1px);box-shadow:0 10px 24px rgba(15,23,42,.08)}
.finance-evidence-frame{height:132px;border-radius:10px;background:#f8fafc;display:flex;align-items:center;justify-content:center;overflow:hidden}
.finance-evidence-frame img{width:100%;height:100%;object-fit:cover}
</style>
@endpush
...
function normalizeAmount(value){return (value||'').toString().replace(/\D/g,'').replace(/^0+(?=\d)/,'');}
function formatRupiah(value){var digits=normalizeAmount(value);return digits?digits.replace(/\B(?=(\d{3})+(?!\d))/g,'.'):'';}
function syncAmount(){var digits=normalizeAmount(document.getElementById('amountDisplay').value);document.getElementById('amount').value=digits;document.getElementById('amountDisplay').value=formatRupiah(digits);}
...
$('#transactionDate').flatpickr({dateFormat:'Y-m-d',altInput:true,altFormat:'d/m/Y',allowInput:true});
...
function filterItemByCategory(){var category=document.getElementById('categorySelect').value;document.querySelectorAll('#itemSelect option[data-category]').forEach(function(option){option.hidden=category&&option.dataset.category!==category;});var selectedItem=document.getElementById('itemSelect').value;if(selectedItem&&document.querySelector('#itemSelect option[value="'+selectedItem+'"]').hidden){document.getElementById('itemSelect').value='';}}
document.getElementById('categorySelect').addEventListener('change',filterItemByCategory);
filterItemByCategory();syncAmount();
function syncEvidenceInput(){var dataTransfer=new DataTransfer();selectedEvidenceFiles.forEach(function(file){dataTransfer.items.add(file);});document.getElementById('evidenceInput').files=dataTransfer.files;}
...
function renderEvidencePreview(){evidenceObjectUrls.forEach(function(url){URL.revokeObjectURL(url);});evidenceObjectUrls=[];if(!selectedEvidenceFiles.length){clearEvidencePreview();return;}var list=document.getElementById('evidencePreviewList');list.innerHTML='';selectedEvidenceFiles.forEach(function(file,index){var objectUrl=URL.createObjectURL(file);evidenceObjectUrls.push(objectUrl);var thumb=file.type.indexOf('image/')===0?'<img src="'+objectUrl+'" class="w-100 h-100" style="object-fit:cover;" alt="Preview eviden">':'<span class="badge badge-light-danger">PDF</span>';var col=document.createElement('div');col.className='col-md-6 col-xl-4';col.innerHTML='<div class="border rounded p-3 h-100"><div class="border rounded bg-light d-flex align-items-center justify-content-center mb-3" style="height:120px;overflow:hidden;">'+thumb+'</div><div class="fw-semibold text-truncate" title="'+file.name+'">'+file.name+'</div><div class="text-muted fs-7 mb-3">'+formatFileSize(file.size)+'</div><div class="d-flex gap-2"><button type="button" class="btn btn-sm btn-light-primary evidence-view" data-index="'+index+'">Lihat</button><button type="button" class="btn btn-sm btn-light-danger evidence-remove" data-index="'+index+'">Hapus</button></div></div>';list.appendChild(col);});document.getElementById('evidencePreview').classList.remove('d-none');}
